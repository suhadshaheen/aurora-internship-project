...
export const selectAllComments = createSelector(selectCommentState, (state) => state.comments);
...
export const selectCommentLoading = createSelector(selectCommentState, (state) => state.loading);

export const selectCommentError = createSelector(selectCommentState, (state) => state.error);
...
export const selectCommentById = (commentId: number) =>
  createSelector(
    selectAllComments,
    (comments) => comments.find((comment) => comment.id === commentId) ?? null,
  );

export const selectMainCommentsBySectionId = (sectionId: number) =>
  createSelector(selectAllComments, (comments) =>
    comments.filter(
      (comment) => comment.sectionId === sectionId && comment.parentCommentId === null,
    ),
  );

export const selectRepliesByParentCommentId = (parentCommentId: number) =>
  createSelector(selectAllComments, (comments) =>
    comments.filter((comment) => comment.parentCommentId === parentCommentId),
  );
